...
import bozoware.client.display.notifications.NotificationManager;
...
public class Alt {
    ...
    public void login() {
        ...
        if(cracked || getPassword().equals("")) {
            ...
            displayName = username;
            cracked = true;
            NotificationManager.INSTANCE.push("Cracked Login", "Logged into "+displayName, 5000, NotificationManager.NotificationType.NONE);
        } else {
            new Thread("Alt Login"){
                public void run() {
                    ...
                    try {
                        ...
                        displayName = mc.session.getUsername();
                        invalid = false;
                        NotificationManager.INSTANCE.push("Login Success", "Logged into "+displayName, 5000, NotificationManager.NotificationType.SUCCESS);
                    } catch (AuthenticationException e) {
                        invalid = true;
                        NotificationManager.INSTANCE.push("Login Failed", "Invalid Details", 5000, NotificationManager.NotificationType.ERROR);
                        e.printStackTrace();
                    }
                }
            }.start();
